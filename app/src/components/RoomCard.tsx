      ...
      <div className="p-4">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center space-x-2">
            <Avatar user={room.host} size="sm" showLiveIndicator={room.isLive} />
            <div>
              <p className="font-medium text-gray-900 text-sm">{room.host.displayName}</p>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-1">
              ...
              <span className="text-sm text-gray-600">{room.listenerCount}</span>
            </div>
            <div className="flex items-center space-x-1">
              <Clock className="w-4 h-4 text-gray-500" />
              <span className="text-sm text-gray-600">
                {variant === 'live' ? formatDuration(room.startTime) :
                 variant === 'scheduled' ? formatTime(room.startTime) :
                 'Replay'}
              </span>
            </div>
          </div>
